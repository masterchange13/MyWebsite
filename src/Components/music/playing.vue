            ...
            <div ref="musicLyric" class="music-lyric" :style="{ 'padding-top': paddingTop }">
                <div class="music-lyric-items" :style="lyricTop">
                    <template v-if="lyricInfo.length > 0">
                        <p v-for="(item, index) in lyricInfo" :key="index" :data-index="index" ref="lyric" :style="{
    ...
    setup(props, { emit }) {
        ...
        const lyricTop = computed(() => {
            return `transform :translate3d(0, ${(0 - state.lineHeight) *
                (props.lyricIndex - state.top)}px, 0);color: ${state.color};line-height: ${state.lineHeight
                }px`;
        })
